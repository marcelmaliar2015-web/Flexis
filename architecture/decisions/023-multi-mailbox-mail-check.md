...
A signed-in user may connect any number of Gmail and Outlook mailboxes. Unique key is `(UserId, Provider, ExternalSubject)`. Reconnecting the same account refreshes tokens. Disconnect is per connection id. Mail Check Settings offers Add Gmail, Add Outlook, and Copy URL for each provider so another browser or account can finish OAuth without leaving the page. Check all processes every connected mailbox; Check runs one mailbox. Scan cursors resume past already-checked candidates so large inboxes keep advancing. Inbox is a combined list with mailbox identity on each row. Processed message ids are scoped by `MailConnectionId`. OpenAI settings stay one per user. Outlook connect still requires the admin Microsoft client and `Microsoft:RedirectUri` (`outlookAvailable`).
