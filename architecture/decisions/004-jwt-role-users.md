...
Admin can list, create, update, and delete users of every role. The last active admin cannot be demoted, deactivated, or deleted. The admin UI is on `/settings`. `GET /api/health` stays anonymous. Other API endpoints require a valid token. User and Viewer currently share the same screens; User is the future write role.
