...
Financial lists one row per pipeline entry. Main (row `total` / `ready` / `notReady` / `applied` / `interviews` / `unapplied` / `price`) counts come from that profile's named main tab: total is non-empty listing rows, ready has Download filled, not ready has blank Download, and applied / interviews / unapplied (blank Status) count only among ready rows. Today counts the same fields among listings whose keys are in the latest Update copy batch for that profile (source listings present on main after Update: appended and skipped duplicates). Archived counts sum numbered tabs (`1`, `2`, `3`, …) created by Forward. Status dropdown maintenance skips archived tabs. Lifetime is main plus archived. Price is `applied * applyRate + interviews * bonusRate` for each period, rounded to 2 decimals. Each row has its own apply rate and bonus rate. New pipeline rows copy the user's defaults (0.06 and 1.5 until changed). Defaults are edited on Settings (Job Application), not Admin. Changing defaults does not rewrite existing row rates. Rows are selectable so a subset can be priced in the UI. Profile Status includes Interview.
