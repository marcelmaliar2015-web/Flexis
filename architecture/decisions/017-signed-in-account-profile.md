...
`UserMenu` uses a compact header: avatar, display name, email, and a role chip, then Settings, Help, and Sign out. Settings Account tab always has a Your account card. The signed-in user can update display name, workspace time zone (IANA id), and optional password. Email and role are read-only. `PUT /api/auth/me` performs that update. The Admin users table lists other accounts only. The AppBar clock also edits time zone; see [034-workspace-time-zone.md](034-workspace-time-zone.md).
