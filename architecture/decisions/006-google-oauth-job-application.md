...
Later Sheets and Drive calls reuse the stored refresh token. Profile and source create uses Sheets to make the workbook, then Drive to place it under `Flexis` / `Job Application` / `Profiles` or `Sources`. Do not add a Google client secret to the frontend. Do not request `drive` or `mail.google.com`. The Flexis Google Cloud web client is stored by an admin in Settings; each user still connects their own Gmail. See [009-google-client-in-settings.md](009-google-client-in-settings.md) and [011-google-drive-folder-layout.md](011-google-drive-folder-layout.md). Production needs those APIs enabled and Google verification for Gmail.
