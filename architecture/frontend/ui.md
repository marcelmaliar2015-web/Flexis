...
Authenticated product routes use `AuthenticatedLayout`: a left panel with Dashboard, Job Application, and Settings buttons, then the page `Outlet`. Dashboard and Settings are empty. Job Application uses MUI Tabs. The Settings tab has Gmail connect and tables to add, edit, and delete profiles and sources (`title`, `createdAt`, `url`).
