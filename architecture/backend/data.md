...
`JobFinancialSnapshot` in `Flexis.Domain.JobApplication`. Table `job_financial_snapshots`. Unique (`UserId`, `CapturedHour`) UTC hour, cascade from `users`. Concurrent Financial/Statistics loads upsert the current hour safely. `CapturedOn` is the UTC date of that hour. Stores today (last Update batch), main sheet, archived, and lifetime price and listing counts. EF configuration: `Persistence/Postgres/JobApplication/JobFinancialSnapshotConfiguration.cs`. See [028-financial-performance-snapshots.md](../decisions/028-financial-performance-snapshots.md).
