...
`JobFinancialBoardDto`: `defaults` (`applyRate`, `bonusRate`), `rows`, `allPrice`, `allTotal`, `allApplied`, `allInterviews` (today main tab totals), `archivedAllPrice`, `archivedAllTotal`, `archivedAllApplied`, `archivedAllInterviews` (numbered archive tabs), `lifetimeAllPrice`, `lifetimeAllTotal`, `lifetimeAllApplied`, `lifetimeAllInterviews`, `history` (`JobFinancialSnapshotDto[]`, oldest first, up to 14 days of UTC hours). Loading the board upserts this hour's snapshot. `JobFinancialSnapshotDto`: `capturedOn`, `capturedHour`, `capturedAt`, today/archived/lifetime `price`, `total`, `applied`, `interviews`. `JobFinancialRowDto`: `entryId`, `profileId`, `profileTitle`, `profileUrl`, `sourceLabel`, today fields (`total`, `applied`, `interviews`, `unapplied`, `price`), archived fields (`archivedTotal`, `archivedApplied`, `archivedInterviews`, `archivedUnapplied`, `archivedPrice`), lifetime fields (`lifetimeTotal`, `lifetimeApplied`, `lifetimeInterviews`, `lifetimeUnapplied`, `lifetimePrice`), `applyRate`, `bonusRate`. `unapplied` counts non-empty listing rows with a blank Status. Today counts come from the profile named main tab. `JobStatisticsBoardDto`: `profiles` (`profileId`, `profileTitle`, `profileUrl`, `applied`, `interviews`, `unapplied`, `total`, `price`, `applyRate`, `bonusRate`), `history` (`JobStatisticsPointDto[]`, oldest first, about 93 days of UTC hours), `allApplied`, `allInterviews`, `allUnapplied`, `allTotal`, `allPrice`. Loading statistics upserts per-profile hourly snapshots.. Archived counts sum numbered tabs (`1`, `2`, `3`, …) from Forward. Price is `applied * applyRate + interviews * bonusRate` per period. Defaults PUT and row rates PUT body: `applyRate`, `bonusRate` (0 to 10000). Missing pipeline row: `404`. See [013-job-application-financial-logs.md](../decisions/013-job-application-financial-logs.md) and [028-financial-performance-snapshots.md](../decisions/028-financial-performance-snapshots.md).
