...
| GET | `/api/job-application/financial/history` | Authenticated | `200` + `JobFinancialSnapshotDto[]` |
| GET | `/api/job-application/financial/statistics` | Authenticated | `200` + `JobStatisticsBoardDto` |
...
| GET | `/api/job-application/search` | Authenticated | `200` + `JobSearchMetaDto` |
| POST | `/api/job-application/search` | Authenticated | `200` + `JobSearchResultDto` |
| POST | `/api/job-application/listings/sync` | Authenticated | `200` + `JobListingSyncResultDto` |
| POST | `/api/job-application/listings/reindex` | Authenticated | `200` + `JobListingSyncResultDto` |
| GET | `/api/job-application/logs` | Authenticated | `200` + `JobApplicationLogPageDto` |
...
| GET | `/api/mail-check/usage` | Authenticated | `200` + `MailCheckUsageDto` |
...
| GET | `/api/mail-check/mailbox/outlook/callback` | Anonymous | Redirect to Settings (or legacy Mail Check) with `?mailbox=` |
| DELETE | `/api/mail-check/mailbox/{id}` | Authenticated | `204` |
| POST | `/api/diagnostics/events` | Authenticated | `204` |

`UserDto`: `id`, `email`, `displayName`, `role`, `isActive`, `timeZoneId`, `createdAt`. Role values: `Admin`, `User`, `Viewer`. `timeZoneId` is an IANA zone id or empty (UI then uses the device zone). `PUT /api/auth/me` body: `displayName`, `timeZoneId`, `password` (omit or blank to keep the current password). Email, role, and active cannot change on that path. Admin user PUT body: `displayName`, `role`, `isActive`, `password` (optional).
...
`JobPipelineBoardDto`: `entries`, `profiles`, `sources` (each source includes `locations`). `JobPipelineEntryDto`: `id`, `profileId`, `sourceId`, `locationSheetId`, `locationName`, `createdAt`. Write body: `profileId`, `sourceId`, `locationSheetId`. Duplicate profile and source location: `409`. Delete All removes every pipeline entry for that user. Listings already on profile sheets stay. Update copies Company Name, Position, Link, and JD from that source tab onto the named profile main tab immediately after the last filled listing row, after removing blank rows between filled listings, and skips rows already present (`added`, `skipped`) or banned (`banned`). Each Update replaces that profile's Today copy batch with keys for every non-banned source listing in that Update (appended and skipped duplicates — never the full main sheet). Bans are per profile. Update All does that for every pipeline entry. After Update, invited editors can edit Status and Issue only on rows still present in the current source; other listing rows are fully locked. Forward renames the current main tab to the next unused number (`1`, `2`, `3`, …) and creates a new empty main tab with the original name (`archivedSheetName`, `mainSheetName`). Forward All does that once per distinct profile (`forwarded`). Then it reapplies owner lock; invited editors can still edit Status and Issue on the new named main tab. Numbered log tabs stay owner-only.

`ProfileBannedCompanyDto`: `id`, `companyName`, `createdAt`. Write body: `companyName`. Duplicate after name folding: `409`. Too-generic name: `400`. `ProfileBannedMatchesDto`: `matches` array of `companyName`, `position`, `link`, `matchedBan`. Matches are scanned from that profile's `profile_main` projection rows. Each matching row gets Status `Banned` on the sheet and in the projection. See [012-pipeline-banned-companies.md](../decisions/012-pipeline-banned-companies.md) and [036-job-listing-projection.md](../decisions/036-job-listing-projection.md).

`JobFinancialBoardDto`: `defaults` (`applyRate`, `bonusRate`), `rows`, main totals (`allPrice`, `allTotal`, `allReady`, `allNotReady`, `allApplied`, `allInterviews`, `allUnapplied`), today totals (`todayAll*`), archived totals (`archivedAll*`), lifetime totals (`lifetimeAll*`), `history` (`JobFinancialSnapshotDto[]`, oldest first, up to 14 days of UTC hours). Loading the board reads listing counts from the projection, upserts this hour's snapshot, and syncs listing Status state/events. Board cache TTL is 10 minutes. `JobFinancialSnapshotDto`: `capturedOn`, `capturedHour`, `capturedAt`, today (last Update) / main / archived / lifetime `price`, `total`, `applied`, `interviews`. `JobFinancialRowDto`: `entryId`, `profileId`, `profileTitle`, `profileUrl`, `sourceLabel`, main / today / archived / lifetime fields including `ready`, `notReady`, `applied`, `interviews`, `unapplied`, `price`, plus `applyRate`, `bonusRate`. Ready means Download is filled (Status ignored). Not ready means Download and Status are both blank; a filled Status is not not-ready. Applied, Interview, and Unapplied (blank Status) count only among ready rows. `JobStatisticsBoardDto`: `profiles` (main and today counts including ready/not ready, plus rates), `history` (status-event volumes per profile UTC hour), `all*` (main), `todayAll*` (last Update). Loading statistics upserts per-profile hourly last-Update snapshots. Price is `applied * applyRate + interviews * bonusRate` per period. Defaults PUT and row rates PUT body: `applyRate`, `bonusRate` (0 to 10000). Missing pipeline row: `404`. See [013-job-application-financial-logs.md](../decisions/013-job-application-financial-logs.md), [028-financial-performance-snapshots.md](../decisions/028-financial-performance-snapshots.md), [033-job-application-statistics.md](../decisions/033-job-application-statistics.md), and [036-job-listing-projection.md](../decisions/036-job-listing-projection.md).
...
`JobSearchMetaDto`: `profiles` (`profileId` nullable, `title`), `statuses`, `searchBaseUrl`, `searchBaseSpreadsheetId`. Ensures Drive workbook `search-base` under Job Application with Main columns Company Name, Position, Link, JD, Download, Status, Issue, Profile. `JobSearchRequest` body: optional `profile`, `companyName`, `position`, `link`, `jd`, `status`. `JobSearchResultDto`: `items` (`rank`, `score`, listing fields, `source` `profile`|`profile-archive`|`profile-archive:{tab}`|`search-base`, `spreadsheetUrl`, `profileId`), `scanned`, `searchBaseUrl`. Profile and Status are exact filters when set; text fields use weighted fuzzy ranking against the listing projection (main, archive, and search-base). `JobListingSyncResultDto`: `checked`, `pulled`. Sync pulls only spreadsheets whose Drive `modifiedTime` changed; reindex force-pulls every profile workbook and search-base. See [035-job-application-listing-search.md](../decisions/035-job-application-listing-search.md) and [036-job-listing-projection.md](../decisions/036-job-listing-projection.md).

`JobApplicationLogDto`: `id`, `occurredAt`, `category`, `action`, `summary`, `detail`. `JobApplicationLogPageDto`: `items`, `page`, `pageSize`, `totalCount`, `totalPages`. Query: `page`, `pageSize` (default 50, max 100), optional `category`, optional `q`. Categories: `pipeline`, `catalog`, `financial`, `resume`, `account`.

`MailCheckSettingsDto`: `hasApiKey`, `model`, `classifierPrompt`, `defaultClassifierPrompt`, `labelActions`, `defaultLabelActions`, `needActionLabels`, `defaultNeedActionLabels`, `autoCheckEnabled`, `autoCheckIntervalSeconds` (20), last-run counts, `mailboxes` (`id`, `provider`, `email`, `connectedAt`, `checkedNewestAt`, `checkedUntilAt`, `lastScanAt`, `scanCaughtUp`), `outlookAvailable`. PUT body: `apiKey` (omit or blank to keep), `clearApiKey`, `model`, optional `classifierPrompt`, optional `labelActions` (all twelve label slugs required when sent), optional `needActionLabels` (at least one slug when sent), optional `autoCheckEnabled`. The key is never returned. `MailMailboxStatusDto`: `outlookAvailable`, `mailboxes` (same item shape). `POST /api/mail-check/mailbox/gmail/start` body: `returnUrl` (must be `/settings` or legacy `/mail-check` on an allowed origin). Returns `authorizationUrl`. OAuth callback redirects with `?mailbox=connected|denied|error`. `DELETE /api/mail-check/mailbox/{id}` disconnects that Mail Check mailbox. `MailCheckModelsDto`: `models` (`id`, `recommended`) from OpenAI with the stored key. `MailCheckRunDto`: `busy`, `processed`, `labeled`, `trashed`, `skipped`, `errors`, `hasMore`, `scanned`, `alreadySeen`, `mailboxId`, `mailboxEmail`, `mailboxProvider`, `items` (each item includes `messageId`, classified `label`, mailbox action `action`, mailbox id/email/provider). Each run classifies one new message per mailbox. Run body: `force`, optional `mailboxId` (one mailbox; omit for all), `resetCursor` (restart candidate scan). `MailCheckInboxDto`: pinned messages from every connected mailbox; each item includes mailbox id/email/provider. Query `label` is a pin-configured classifier slug. `GET /api/mail-check/need-action` returns labeled mail whose label is in `needActionLabels` (pin or keep). `GET /api/mail-check/logs` query: `page`, `pageSize` (default 50, max 100), optional `source` (`auto`|`manual`), optional `action`, optional `mailboxId`, optional `q`. Returns `MailCheckActionLogPageDto`: `items`, `page`, `pageSize`, `totalCount`, `totalPages`. Each `MailCheckActionLogDto`: `id`, `runId`, `occurredAt`, `source`, `mailboxId`, `mailboxEmail`, `mailboxProvider`, `messageId`, `subject`, `from`, `action`, `label`, `detail`, `durationMs`. Actions include `pin`, `trash`, `keep`, `already_checked`, `error`, and `run_completed`. `GET /api/mail-check/usage` returns `MailCheckUsageDto`: `lifetime` and `today` totals (`callCount`, `promptTokens`, `completionTokens`, `totalTokens`, `estimatedCostUsd`) plus `history` (up to 14 days of hourly points, oldest first: `capturedOn`, `capturedHour`, `updatedAt`, same counters, `lastModel`). Cost is estimated from OpenAI list rates. No mailbox connected or missing OpenAI key: `400`. See [019-mail-check.md](../decisions/019-mail-check.md), [023-multi-mailbox-mail-check.md](../decisions/023-multi-mailbox-mail-check.md), [026-mail-check-action-log.md](../decisions/026-mail-check-action-log.md), and [029-mail-check-openai-usage.md](../decisions/029-mail-check-openai-usage.md).
...
- [../decisions/029-mail-check-openai-usage.md](../decisions/029-mail-check-openai-usage.md)
- [../decisions/035-job-application-listing-search.md](../decisions/035-job-application-listing-search.md)
- [../decisions/036-job-listing-projection.md](../decisions/036-job-listing-projection.md)
