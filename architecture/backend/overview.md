...
- `Flexis.Domain` — entities and domain rules, including `User`, `GoogleConnection`, `GoogleClientCredentials`, `JobCatalogItem`, `JobPipelineEntry`, `JobProfileBannedCompany`, `JobFinancialSettings`, `JobFinancialSnapshot`, `JobApplicationLog`, `MailCheckSettings`, `MailCheckProcessedMessage`, and `MailCheckActionLog`.
