...
- `Flexis.Domain` — entities and domain rules, including `User`, `GoogleConnection`, `GoogleClientCredentials`, `JobCatalogItem`, `JobPipelineEntry`, `JobPipelineBannedCompany`, `JobFinancialSettings`, `JobApplicationLog`, `MailCheckSettings`, and `MailCheckProcessedMessage`.
- `Flexis.Application` — use cases, DTOs, DI entry `AddApplication`.
- `Flexis.Infrastructure` — EF Core, MongoDB, JWT, password hashing, Google OAuth, Google Sheets, Google Drive, Gmail, OpenAI, health checks, DI entry `AddInfrastructure`.
