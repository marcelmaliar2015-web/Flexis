...
{
  "label": "rejected | applied | schedule | scheduled | assessment | availability | ai_interview | code | success | need_action | other | less_important"
}
...
- need_action: use when you cannot confidently choose among the labels above, the mail is ambiguous or conflicting, or the mail needs important human attention or a decision that is not covered by schedule, scheduled, assessment, availability, ai_interview, code, or success
- other: job-related mail about an application or interview you are actively pursuing that clearly does not fit the specific labels above and does not need urgent human triage (for example routine background check steps, onboarding tasks, or HR paperwork for a role in process)
...
- Prefer the most specific label that fits when you are confident.
...
- Prefer code over less_important, other, need_action, applied, and success for authentication and verification mail, including codes from employers, ATS portals, Google, Microsoft, LinkedIn, GitHub, or banks.
...
- Prefer less_important over other and need_action for bulk alerts, digests, and suggestions you did not solicit for a specific application.
...
- Prefer need_action over other when the mail needs the user to notice, decide, reply, or act and no specific action label fits, or when two or more labels could apply and the choice is unclear.
- Use other only when the mail is clearly job-related for an active process, you are confident it is routine process mail, and it does not need urgent human triage.
- Use need_action rather than guessing when the body is incomplete, contradictory, or too vague to classify confidently.
...
- Subject "Please review and reply" with unclear next steps that matter for an active application -> need_action
- Subject "Important update" whose body could be schedule or assessment but is too vague to tell -> need_action
